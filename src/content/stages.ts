...
export const STAGES: Stage[] = [
...

  {
    ...
    brief: [
      'You are the engineer on the procurement call. Four controllers, and the table in front of you has the two columns a manager always asks for: how often does it break the rules, and what does it cost.',
      'One of them has the best safety score on the board and is the only one that makes money.',
      'Choose the one you would put on a real feeder.',
    ],
    task: 'Read the table. Choose one.',
    mode: 'choose',
    unlocks: ['grid', 'loadScale', 'compare', 'margin'],
    revealTitle: 'The third column',
    reveal: () => [
      'The controller with the best violation rate and the only negative cost served nobody. Not one vehicle reached its target, all day.',
      'It achieved a perfect safety record by never operating. And because every request in this scenario was physically deliverable, that is not a limitation of the grid — it is the controller declining to do its job.',
      'This is the failure that never shows up in the field, because reality does not hand you the counterfactual. An operator who deploys this gets a report saying violations are at an all-time low and costs are negative, and no report at all saying two hundred and eighty-seven people could not get to work.',
