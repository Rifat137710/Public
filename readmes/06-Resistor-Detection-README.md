...
- **Finds every resistor in a photograph of a populated board, reads its colour bands, and prints
  its resistance value** — regardless of how the resistors are rotated or flipped, and no matter
  how many are in the frame.
- **Automates a task that is slow, subjective and error-prone by eye,** especially for beginners
  and for anyone with colour-vision deficiency.
- **Segments the image in stages:** RGB → greyscale → **global thresholding** → binarise and
  invert (white objects on black) → morphological cleanup of the black artefacts left inside the
  white regions.
- **Adapts to lighting automatically.** The image is also converted to **HSV**, and the mean of
  the value (brightness) channel selects the structuring element for morphological opening — so
  the pipeline self-adjusts to bright or dim photographs instead of relying on one fixed kernel.
  Region properties (eccentricity, area, bounding box) are then computed for the survivors.
- **Identifies resistors by body colour.** A custom `maskedYellow` function exploits the fact that
  a resistor's yellowish-brown body is unique on the board — no MOSFET, BJT, LED or wire shares
  it — so body colour alone separates resistors from everything else before any band reading begins.
- **Reads bands and resolves reading direction.** Per-colour mask functions (red, black, brown,
  orange) locate each band and return its pixel positions; band coordinates are then summed and
  compared against an empirically tuned threshold of **250** to decide orientation, covering all
  four cases — horizontal and vertical, flipped and unflipped — before applying
  `Resistance = (Band1 Band2) × 10^Band3`.
- **Result — correct on every test case.** Verified on a 1 kΩ horizontally flipped, a 1 kΩ
  vertical unflipped, a 1 kΩ and 220 Ω pair, and mixed boards where only 2 of several components
  were resistors. In each case the resistors were isolated, boxed in red on the source photo, and
  valued correctly.
- **Result — black bands are the known weak point.** A phone camera never captures pure black, so
  that threshold is hard to calibrate and the band is occasionally missed. Testing covered three
  resistor values only.
- **Result — the capture setup bounds accuracy.** A uniform white background, perpendicular shot,
  high resolution and absence of shadows are all required; the white background is a mitigation
  for lighting sensitivity, not a fix.
