...
- **Built a mains-powered battery charger with an adjustable DC output and an automatic cut-off**
  that stops charging the instant the battery is full — so one charger serves Li-ion, LiPo and
  lead-acid batteries instead of needing a separate fixed-voltage unit per device.
- **Attacked two problems at once:** fixed-voltage chargers only fit one battery type, so users
  end up owning several; manual chargers need someone watching the process, and overcharging
  degrades capacity and can become a safety hazard.
- **Built the complete power chain** from wall socket to battery — 230 V AC → 220/12 V
  centre-tapped transformer (18.33:1, 3000 mA) → full-wave bridge rectifier → LC filter
  (100 mH series, 1000 µF shunt) → **IRF250N** buck stage → relay → battery — with an **SG3524**
  IC generating the switching pulses and a potentiometer trimming the duty cycle.
- **Chose a switching converter over a linear regulator** as the central efficiency decision: a
  linear regulator burns the surplus voltage as heat, while the buck stage passes it through.
- **Implemented the cut-off in Arduino firmware** that averages **five successive ADC readings**
  from both the reference and battery channels to reject noise, then acts on their ratio
  `r = reference / battery` to drive a relay through a MOSFET.
- **Result — the buck stage tracks its design equation.** Verified on an oscilloscope at **25%,
  50% and 90% duty cycle**, the output followed `V_o = k · V_in` across the full range.
- **Result — the cut-off works and does not chatter.** Two different thresholds — 1.55 while
  charging, 2.40 once cut off — create deliberate **hysteresis**, so a battery hovering near full
  does not oscillate the relay on and off. An upper bound of 4.33 guards against a disconnected
  or shorted battery. Charging to full and observing the halt confirmed the behaviour.
- **Result — measurably more efficient.** Power measurements showed the switching design wasted
  markedly less energy, and ran cooler, than a linear-regulator equivalent.
- **Result — one stage could not be demonstrated.** Lab facility limits meant the transformer
  stage was left out of the final demonstration, and the absence of high-precision oscilloscopes
  and power analysers capped how thoroughly performance could be characterised.
